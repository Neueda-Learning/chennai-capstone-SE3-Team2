...
/** Story 613 -- retry vs dead-letter. Kafka is at-least-once. */
@Configuration
public class ConsumerErrorHandling {
     ...
     * assignable to one of these is classified as poison and dead- lettered on
     * the first attempt.
     */
    ...
    /**
     * The poison exceptions we recognise BY NAME.
     * DefaultErrorHandler#defaultFalse below), so this list does not decide
     * 'is it poison'.
     */
    ...
    public DefaultErrorHandler orderPlacedErrorHandler(
        ...
        // Default: not retryable. Anything on the transient list below is
        // added back on.
        ...
        // otherwise a container restart re-delivers the poison and it dead-
    ...
    /**
     * The header set the DLT recoverer stamps on every dead-lettered record.
     * Package-private and static so the unit test drives the same code the
     * production handler does, not a copy.
     */
     ...
     * exception and answers 'is this transient?'.
     */
