...
/**
 * The guarded state transition against a real PostgreSQL. This is the one part
 * of the duplicate mechanism that cannot be proved with a double.
 */
@SpringBootTest
@EnabledIf(
...
class GuardedSettlementIntegrationTest extends ExecutorPostgresSupport {
    ...
    void secondDeliveryAffectsZeroRows() {
        ...
        // The same message again: the executor is handed the same order
        // twice during a rebalance, after a crash between the database
    ...
    void concurrentDeliveriesAreSerialisedByTheDatabase() throws Exception {
        ...
        // would let several of these through, because every read would see
    ...
    void reasonOnAFilledOrderIsRefused() {
        // ck_orders_reason_matches_status, from migration 004. A FILLED
        // order carrying INSUFFICIENT_FUNDS is a contradiction, and the
