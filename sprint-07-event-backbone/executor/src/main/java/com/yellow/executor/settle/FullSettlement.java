 ...
 * atomic transaction.
 *
 * <p>Three writes, always in this order:
 * <ol>
 *   <li>{@code UPDATE orders WHERE status = 'NEW'} -- the duplicate guard.
 *       Zero rows → ALREADY_SETTLED; return immediately, touch nothing.</li>
 *   <li>{@code UPDATE client_account WHERE version = #{expected}} -- the
 *       optimistic lock. Re-read and retry up to {@link #MAX_LOCK_ATTEMPTS}
 *       times. Exhausted → throw {@link LockExhaustedException} so
 *       {@code @Transactional} rolls back the orders write, leaving the order
 *       at NEW for the next delivery.</li>
 *   <li>INSERT / UPDATE / DELETE on {@code position}.</li>
 * </ol>
 *
 * <p>The orders write is first so that a concurrent delivery hitting step 1 at
 * any point gets zero rows and returns ALREADY_SETTLED without touching the
 * account. Without that ordering, two deliveries could each pass step 1 and
 * each proceed to write the account twice.
 */
@Component
public class FullSettlement implements SettlementPort {
    ...
    private void writePosition(OrderSnapshot order, BigDecimal executedPrice) {
        ...
        if (order.isBuy()) {
            ...
            if (existing == null) {
                // PreTradeChecks.atExecution should have rejected this order.
                // Logging the anomaly here is defensive; the fill already wrote
                // to orders and client_account, so we note it and move on.
