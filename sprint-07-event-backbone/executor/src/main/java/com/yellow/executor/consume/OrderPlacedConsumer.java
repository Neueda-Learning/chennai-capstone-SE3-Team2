...
/**
 * Reads accepted orders off {@code orders} and hands each to the executor.
 *
 * <p>THE CONSUMER GROUP IS FIXED BY CONTRACT as {@code trade-executor}, and
 * {@code orders} has exactly one logical consumer group because it is a work
 * queue. A second group would mean two services executing the same order. Three
 * partitions means up to three instances of this can usefully share the work; a
 * fourth would sit idle, because a consumer group cannot exceed the partition
 * count.
 *
 * <p>AUTO-COMMIT IS OFF and the offset is acknowledged only once the order has
 * been dealt with. Committing first would lose an order to a crash; committing
 * after means a crash reprocesses it. Reprocessing is survivable because the
 * settlement is idempotent, and losing a trade is not.
 */
@Component
public class OrderPlacedConsumer {
    ...
    public void onOrderPlaced(EventEnvelope<OrderPlacedPayload> envelope,
        ...
        // dead-letters it rather than retrying, because a message whose shape
        // we cannot read will not become readable on a second attempt.
