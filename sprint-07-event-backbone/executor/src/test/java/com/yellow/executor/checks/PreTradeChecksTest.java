...
/**
 * The checks around the fill rule.
 *
 * <p>The re-check tests all describe the same shape of event: something was
 * true when the order was accepted and is not true now. That gap is the whole
 * reason the executor re-checks rather than trusting what the Trade REST API
 * already validated -- an order can sit on a topic for as long as the executor
 * is behind, and a week of Kafka retention is a long time for a balance to
 * stay still.
 */
class PreTradeChecksTest {
    ...
    class BeforePricing {
        ...
        void mutualFundIsNotPriceable() {
            // Fauxnance's registry is closed at equity, etf, fx and crypto, so
            // this would 404. More to the point, a fund has no bid or ask at
            // all: units are allotted at a NAV struck after close.
    ...
    class AtExecution {
        ...
        void reCheckUsesExecutedPrice() {
            // Limit 126,000 x 4 = 504,000, which fits 503,000 only if you use
            // the executed price of 125,121.48 x 4 = 500,485.92. Checking
            // against the limit would reject an order that is affordable.
        ...
        void affordabilityIsNetOfBlockedFunds() {
            // Account 3 in the seed: 750,000 balance, 220,000 blocked. An order
            // costing 600,000 fits the balance and not the available funds, and
            // the blocked money is committed to somebody else's order.
