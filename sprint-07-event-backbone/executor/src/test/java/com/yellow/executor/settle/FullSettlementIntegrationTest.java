 ...
 *
 * <p>The test exercises {@link SettlementPort} directly, bypassing
 * {@code OrderExecutionService} and Kafka. The {@code KafkaTemplate} is mocked
 * so that Spring does not attempt a real broker connection during context
 * startup.
 */
@SpringBootTest
@EnabledIf(
...
class FullSettlementIntegrationTest extends ExecutorPostgresSupport {
    ...
    void rebuild() {
        ...
        // releases it. placeOrderAtNew uses price=420, qty=10 → 4200.
