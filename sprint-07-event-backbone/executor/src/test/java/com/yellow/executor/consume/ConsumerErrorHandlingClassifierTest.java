...
/**
 * The header enricher, in isolation.
 *
 * <p>What the review reads is what the recoverer stamps on a DLT
 * record, and that is what {@link ConsumerErrorHandling#buildHeaders}
 * decides. Everything else in the config -- the backoff, the
 * classifier default -- is Spring Kafka wiring, and the integration
 * test proves the wiring holds. This test proves the classification
 * itself, cheaply.
 *
 * <p>Two things are asserted for every case:
 *
 * <ol>
 *   <li>the {@code x-failure-class} answer matches the design decision
 *       (transient is only the three exception types the brief names;
 *       everything else is poison, including a novel bug);</li>
 *   <li>the {@code x-attempt-count} matches what the {@link
 *       org.springframework.kafka.listener.DefaultErrorHandler} would
 *       actually have done -- one attempt for poison, {@code
 *       MAX_RETRIES + 1} for transient. If those two disagree, the
 *       header lies to a teammate reading it on Monday morning, and
 *       that is the failure mode this test exists to prevent.</li>
 * </ol>
 */
class ConsumerErrorHandlingClassifierTest {
    ...
    void anUnknownExceptionIsPoisonByDefault() {
        ...
        // poison. An unclassified exception surfacing as a DLT record
        // and a page is safer than a partition spinning against a
        // record that will never work.
    ...
    void transientDataAccessResourceIsTransient() {
        // The typed exception Spring's SQLExceptionSubclassTranslator
        // raises for a transient connection failure -- SQLSTATE 08*.
        // This is the class that has to be retryable if 'lost DB
        // connection' is going to be handled the way the brief says.
        // (Spring's DataAccessResourceFailureException is
        // Non-Transient in the same hierarchy despite its name -- we
        // ship that one to the DLT rather than pretend otherwise.)
    ...
    void kafkaRetriableIsTransient() {
        ...
        // Reproduces the "broker briefly unreachable" class the brief names.
    ...
    void lockExhaustedIsTransient() {
        // The optimistic-lock budget inside FullSettlement is the CAS
        // retry. When THAT budget is spent, LockExhaustedException
        // bubbles up as an outer-level transient failure: the record
        // will land on a partition where the account is quieter next
        // time.
    ...
    void causeChainIsWalked() {
        // Real production shape: Spring wraps whatever the listener
        // threw in a ListenerExecutionFailedException, whose cause is
        // the actual exception. Classification has to unwrap.
