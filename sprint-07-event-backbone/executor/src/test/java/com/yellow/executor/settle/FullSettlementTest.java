...
/**
 * Unit test for the optimistic-lock exhaustion path.
 *
 * <p>The integration test covers the happy path against a real database. This
 * test covers the one scenario that is hard to reproduce there: an account that
 * is always being updated by another thread, so our lock is never acquired.
 *
 * <p>There is no Spring context here, so {@code @Transactional} on
 * {@link FullSettlement#settle} is an inert annotation. The transaction and its
 * rollback are what the integration environment tests. What this test proves is
 * simpler and more precise: that after {@link FullSettlement#MAX_LOCK_ATTEMPTS}
 * failed CAS attempts the method throws {@link LockExhaustedException} rather
 * than returning any result.
 */
class FullSettlementTest {
    ...
    void lockExhaustedAfterMaxAttempts() {
        ...
        // In production the @Transactional proxy catches this unchecked exception
        // and rolls back the settleIfNew UPDATE, leaving the order at NEW so the
        // message is redelivered. Here we just verify the exception escapes.
