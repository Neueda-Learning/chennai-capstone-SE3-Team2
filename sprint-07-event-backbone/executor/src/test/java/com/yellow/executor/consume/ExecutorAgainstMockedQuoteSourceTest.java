 ...
 * asks for.
 *
 * <p>Real components throughout except the database: the real quote client
 * making real HTTP calls to a real (stubbed) server, the real fill rule, the
 * real checks and the real guarded settlement. The mapper is an in-memory
 * double, so the test runs on any machine without Docker. The one thing it
 * therefore cannot prove is that the conditional UPDATE is serialised by the
 * database, which is why {@code GuardedSettlementIntegrationTest} exists
 * alongside it and needs a real Postgres.
 */
