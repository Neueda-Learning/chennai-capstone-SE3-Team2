...
Schema state: `000_base_schema.sql` + `001_schema_migrations.sql`
+ `002_api_alignment.sql` applied.
...
| `account_ref` | VARCHAR(20), UK | String business reference a customer quotes to support. This is what the API contract calls `AccountResponse.accountId` — the one place that name means a string, not the numeric key |
...
| Column | Type | Meaning |
|---|---|---|
| `order_id` | UUID, PK | Minted by the domain in `Order.place()` before the insert, not by the database, so a retry can carry the identity it was given. Carried over to history on archival, so unique across both |
| `client_id` | INTEGER, FK | Who placed it |
| `instrument_id` | INTEGER, FK | What was ordered |
| `side` | VARCHAR(4) | BUY / SELL |
| `order_type` | VARCHAR(10) | MARKET / LIMIT. Defaults to MARKET: `PlaceOrderRequest` has no such field, and on the trading screen buying is one click |
| `product_type` | VARCHAR(3) | MIS (intraday) / CNC (delivery). Determines the resulting `position_type`. Defaults to CNC: no intraday square-off job exists yet, so every position this API creates is DELIVERY |
...
| `status` | VARCHAR(10) | NEW / FILLED / REJECTED / CANCELLED. Fixed by the API contract; the same literals appear in the domain enum and in the generated Angular types. There is no partial-fill literal |
| `idempotency_key` | VARCHAR(64) | Caller-generated; unique per client |
| `date_placed` | TIMESTAMPTZ | Submission time |
| `resolved_at` | TIMESTAMPTZ | When the black box resolved it. NULL iff PENDING |

Constraints: LIMIT requires price; FILLED requires fill_price; `resolved_at`
is NULL exactly when status is NEW.
...
Status is restricted to terminal values only: FILLED / REJECTED / CANCELLED.
NEW is absent on purpose: an order still working has not finished and must
not be archived.
